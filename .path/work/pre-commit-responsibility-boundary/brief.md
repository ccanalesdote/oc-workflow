...
- Allow explicit user risk acceptance to close only a validation gap caused by unavailable validation capability, without requiring attached evidence; acceptance cannot waive known defects, omitted available checks, or undefined safety/compatibility/migration strategy.
...
- Implementation must start only after `.path/work/architect-kernel-playbooks/` reaches its final checkpoint and its intended changes are present in the implementation baseline. Before T-001 opens, `progress.md` for this plan must record (a) the prerequisite final Reviewer verdict and checkpoint ID and (b) either the containing commit hash verified as present in the current baseline, or the current branch/HEAD plus scoped status/diff evidence showing the prerequisite changes are present and no prerequisite session is still writing overlapping files. T-005 inherits this gate through its dependency on T-001 and must re-check the recorded baseline if HEAD or the working tree changed materially.
...
- Destructive, security-sensitive, or otherwise high-risk work without an available adequate validation mechanism requires an explicit user decision before proceeding. That decision authorizes closure of the unavailable-validation gap only and requires no evidence attachment; it cannot waive a known defect, a relevant available validation that was omitted or failed, or an undefined security, compatibility, rollback/compensation, or migration strategy.

## Decisions

- The opencode-path lifecycle ends after commits are created and reported.
- Push guidance, worktree/branch cleanup guidance, and all other post-commit execution ownership are removed from the agent completion contract. Developer does not run or recommend those actions as part of close/finish.
...
- `templates/developer.md`: preserve explicit close intent, pre-commit working-tree/worktree safety, and logical commits, but terminate the workflow immediately after reporting created commits. Remove both push recommendations and post-commit worktree/branch cleanup recommendations; state that push, cleanup, PRs, deployment, environment operations, and activation are outside responsibility. During implementation, use required validation selected from existing project capabilities; do not turn absent infrastructure or post-commit operational evidence into a blocker.
- `templates/reviewer.md`: fail for missing required local implementation, relevant existing validation that was required but skipped or failed, contract violations, known security/correctness defects, undefined required safety/compatibility/migration strategy, or diff hygiene. Do not fail solely because the repository lacks a test/E2E platform or because post-commit migration/deployment evidence is absent when the handoff records that limitation or a valid explicit user risk decision. Risk acceptance never changes the verdict for the listed local deficiencies.
- `templates/auditor.md`: remain an optional, user-invoked local closure audit. Audit claims against available local evidence and valid recorded risk decisions; reject acceptance used to hide known defects, omitted/failed available checks, or undefined required strategy. Do not require post-commit operations or make Auditor mandatory for commit closure.
...
- Tests: assert the lifecycle boundary, optional Auditor, capability-aware validation, migration execution boundary, explicit risk acceptance and its negative limits, and concise-chat/full-handoff distinction without brittle word-count assertions. Add concrete negative cases proving that a known defect, a failed or omitted relevant available check, and an undefined security/compatibility/migration strategy remain blocking despite risk-acceptance language.
...
- User risk acceptance closes only a validation mechanism/evidence gap caused by unavailable capability. It does not override correctness, known defects, failed or skipped available validation, or unresolved security, compatibility, rollback/compensation, and migration decisions.
...
- Explicit user acceptance is sufficient to proceed with a disclosed high-risk validation gap caused by unavailable capability; no attached proof is required. It is not a general waiver for defects, available checks, or missing strategy.
...
3. For a sensitive change whose adequate validation mechanism is unavailable, Architect asks for an explicit user decision and records the accepted validation gap in the persistent handoff when one exists. Before offering acceptance, Architect must confirm there is no known defect being waived, no relevant available check omitted or failed, and no undefined security, compatibility, rollback/compensation, or migration strategy.
...
8. On explicit close/finish intent, Developer performs all working-tree/worktree safety and validation checks before committing, creates logical commits, reports their hashes/messages, and stops immediately. It does not push, recommend push, run or recommend worktree/branch cleanup, or guide any other post-commit operation as part of closure.
...
- AC-01: Within this feature's explicitly targeted lifecycle definitions — `templates/developer.md`, `templates/reviewer.md`, `templates/auditor.md`, `templates/skills/test-strategy/SKILL.md`, `templates/skills/migration-and-data-change/SKILL.md`, and `templates/skills/api-contracts/SKILL.md` — responsibility is bounded to local pre-commit work and commit creation as applicable; push, PR, QA, deployment, production, activation, and operational receipts are outside closure responsibility. This criterion makes no claim about untargeted agent or skill definitions.
- AC-02: Developer's explicit close/finish flow completes all safety checks before commit, creates and reports logical commits, and then stops without running or recommending push, worktree removal, branch deletion, or any other post-commit operation as part of opencode-path closure.
...
- AC-06: A sensitive change with an unavailable adequate validation mechanism requires an explicit user decision; recorded acceptance permits closure of that validation gap without attached evidence, but cannot waive known defects, failed or omitted relevant available checks, or undefined security, compatibility, rollback/compensation, or migration strategy. Regression coverage includes negative cases for each prohibited waiver class.
...
- A security-sensitive or destructive change lacks an available adequate validation mechanism: stop for explicit user acceptance; after acceptance, record the residual validation risk and proceed without demanding attachments only if there is no known defect, omitted/failed available check, or undefined safety/compatibility/migration strategy.
...
- The user asks for worktree removal or branch deletion after commits: those actions are outside the close/finish contract; Developer stops after reporting commits and does not execute or recommend cleanup as part of this workflow.
