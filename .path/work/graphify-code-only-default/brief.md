...
## Scope
- Change `opencode-path graphify` so the default graph refresh path uses Graphify's code-only/local extraction mode.
...
- Graphify's current CLI has indicated that `--code-only` is the escape hatch for code-only extraction without an LLM API key; Developer must preserve child-process argument arrays and must not use shell strings.

## Decisions
- `opencode-path graphify` will use code-only/local mode by default.
...
- `src/lib/graphify.ts`: update Graphify init/update argument builders and state shape/write behavior for code-only mode.
- `src/commands/graphify.ts`: update runtime messaging and preserve existing init/update/force behavior while using code-only mode.
- `src/lib/graphify.test.ts`: update helper tests for code-only args and state mode metadata.
- `src/commands/graphify.test.ts`: update command tests for runtime messaging, code-only init/update/force calls, state writes, and failure behavior.
...
- `src/lib/graphify.ts` must:
  ...
  - Run graph initialization as `graphify . --code-only` instead of `graphify .`.
  - Run graph update as `graphify update . --code-only` instead of `graphify update .`.
  - Run forced update as `graphify update . --force --code-only` or `graphify update . --code-only --force` using the argument order that the installed Graphify CLI supports. Developer must verify supported ordering locally before finalizing. If Graphify rejects both safe code-only update forms, block and escalate; do not silently fall back to semantic update.
  - Continue using `execFile`/`execFileSync` with argument arrays; do not use shell strings.
  - Add state metadata for the mode used by `opencode-path graphify`, using a stable field such as `graphifyMode: "code-only"` or `mode: "code-only"`. Prefer `graphifyMode` to avoid ambiguity with future state fields.
...
- `src/commands/graphify.ts` must:
  ...
  - Print a concise mode line before running Graphify, materially equivalent to: `Mode: code-only — local AST extraction, no LLM/API keys.`
...
- Default mode for `opencode-path graphify`: code-only/local.
...
- Command prints that it is using code-only/local mode and that semantic docs/media extraction should be run directly with Graphify.
- If `graphify-out/graph.json` is absent, command runs Graphify initialization in code-only mode.
- If `graphify-out/graph.json` exists, command runs Graphify update in code-only mode.
- If `--force` is passed and a graph exists, command runs Graphify forced update in code-only mode.
...
### Escalation contract
If Developer finds that the installed/supported Graphify CLI does not accept `--code-only` for update or force-update forms, Developer must stop the affected Graphify command work, record the block in `progress.md` with Task/checkpoint, Problem, Evidence, Impact, Proposed options, and Status `blocked awaiting Architect decision`, and await an Architect decision. Developer must not silently fall back to semantic/full extraction, add a semantic flag, ask for API keys, or pass through arbitrary Graphify arguments. Any material decision change must be persisted to `brief.md` and/or `tasks.md`; decisions only in `progress.md` are not binding.
...
## Acceptance Criteria
- AC-01: `opencode-path graphify` initializes a missing graph using Graphify code-only/local mode and does not require an LLM API key for code-only repositories.
- AC-02: `opencode-path graphify` updates an existing graph using Graphify code-only/local mode; `--force` preserves forced update behavior while remaining code-only.
- AC-03: Runtime output clearly states code-only/local mode and briefly directs users who want semantic docs/media extraction to use the direct `graphify` CLI.
...
- AC-10: Automated tests cover code-only init/update/force args, runtime messaging, no state write on failure, state mode metadata, and no semantic/API-key behavior in `opencode-path graphify`.
...
- Graphify code-only init fails: report failure and do not write state.
- Graphify code-only update fails: report failure and do not write state; do not retry in semantic mode.
- Existing graph was previously built semantically: `opencode-path graphify` still performs future refreshes in code-only mode unless a future design changes this.
...
- Repo contains docs/images/PDFs: default `opencode-path graphify` must not require an LLM API key because it uses code-only mode.
...
- Graphify rejects the chosen `--code-only` ordering for update/force: block and escalate rather than falling back to LLM-backed extraction.

## Open questions
None.

## Assumptions and residual risks
- Graphify supports `--code-only` on the command forms needed for initialization and update in the compatible release line. If implementation proves otherwise, the escalation contract applies.
